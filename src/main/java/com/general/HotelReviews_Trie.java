...
public class HotelReviews_Trie {
	static class Trie {
	...
	public static void main(String[] args) {
		...
		String []B = new String []{"ldvgylw_b_sbm_o_xtlhcsm_xo_gmgnkeu_dxot_gb_x_eyanfe_cukep_s_kljuggg_kjd_z_enpe_qgxiepj_r_z_azuj_lchhbfq_kim_zobiw_bx_uunfsks_s_tekmqd_yzje_uhms_qcoziji_fi_need_ps_r_av_mmt_tbdzqs_e_uvnpp_suacbaz_x_hec_h_egrpu_kdm_pp_eqt_j_par_owz_qyo_ytjbbh_wdy_cprjbx_ho_hp_wq_uhrqz_nbnf_v_nqq_rz_pxiog_li_xdzu_osrkrus_ojbrzm_zpow_jil_fra_md_gp_puu_gxpqnjw_mwax_mns_hh_qqrz_dlt_zo_cjt_zxuglsd_mzcn_ckvfaj_rmxot_o_kbjzwu_wljfrim_myhchz_iwkbarx_gfc_ceyhju_i_wtbvtre", 
		...
		ArrayList<Integer> res = solve(A,inp);
		System.out.println(res);
	}
	
	 public static ArrayList<Integer> solve(String A, ArrayList<String> B) {
	        Trie root = new Trie();
	        List<Map.Entry<Integer,Integer>> set = new ArrayList<>();
	        Comparator comp = new Comparator<Map.Entry<Integer,Integer>>(){
	...
	public static void buildTrie(String str,Trie root){
		if(str.length() == 0){
			root.isEndOfWord = true;
			return ;
		}
		Trie node = root.children.get(str.charAt(0));
		if(node == null){
			node = new Trie();
			char ch = str.charAt(0);
			root.children.put(ch, node);
		}
		buildTrie(str.substring(1), node);
	}
	public static boolean search(String str,Trie root){
		...
		if(node !=null && node.isEndOfWord == true)
			return true;
		return search(str.substring(1), node);
	}
}
